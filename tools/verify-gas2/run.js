// tools/verify-gas2/run.js — chạy toàn bộ test của tầng pure trong gas2/.
//
//   node tools/verify-gas2/run.js           chỉ test đơn vị (nhanh, không cần dữ liệu thật)
//   node tools/verify-gas2/run.js --data    chạy thêm test đối chiếu trên layout ガワ thật
//                                           (cần chạy python tools/verify/exportFixtures.py trước)
//
// Bản sao của tools/verify/run.js với đúng 2 hằng số đổi (PURE_FILES, FIXTURES_DIR).
// Harness của 2 GAS cố tình KHÔNG dùng chung thư mục fixture: chúng đọc những sheet khác
// nhau, và dùng chung là mời gọi việc sửa fixture cho GAS❷ làm đỏ test của GAS❶.
//
// File này gộp cả 3 việc — nạp gas2/, so sánh giá trị, chạy test — vì cả 3 chỉ
// phục vụ đúng một câu hỏi: "logic nghiệp vụ có còn đúng không". Test thật nằm ở
// tools/verify-gas2/tests.js.
//
// VÌ SAO NẠP gas2/ QUA vm CHỨ KHÔNG require: các file trong gas2/ không có
...
// không phải sửa gas2/ chỉ để test được.
//
// gas2/io.js CÓ trong danh sách dù nó dùng SpreadsheetApp/UrlFetchApp: mọi lời gọi API đó
...
// (readCustomerMaster, readTitleMaster, writeTitleMaster, appendLogEntry, notifySlack...)
// — chúng ném ReferenceError vì SpreadsheetApp không tồn tại ở đây, và đó là hành vi đúng.
// gas2/main.js vẫn KHÔNG nạp: nó là tầng dàn dựng, kiểm bằng hàm probe_* chạy tay
...
const PURE_FILES = ['gas2/common.js', 'gas2/config.js'];
